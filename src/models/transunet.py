# =============================================================================
# Thư mục: src/models/transunet.py
# Chức năng: Triển khai cấu trúc mô hình lai TransUNet (kết hợp ResNetV2 và Vision Transformer).
# Lớp quan trọng:
#   - TransUNetConfig: Lớp cấu hình phân cấp cho TransUNet.
#   - TransUNet: Lớp mô hình chính thực hiện tích hợp ViT và nhánh giải mã U-Net.
# =============================================================================
...
class TransUNetConfig:
    """
    Lớp cấu hình tĩnh chứa các tham số chạy cho TransUNet.

    Các tham số đầu vào:
        patches_grid: Lưới phân chia patch cho các đặc trưng lai.
        hidden_size: Kích thước ẩn của Transformer.
        transformer: Các cấu hình siêu tham số cho khối Transformer.
        classifier: Chế độ phân loại (được giữ lại để đảm bảo tính tương thích với checkpoint gốc).
        decoder_channels: Số lượng kênh đặc trưng trong decoder đi từ thô đến mịn.
        skip_channels: Số lượng kênh cho các kết nối tắt (skip connections).
        n_skip: Số cấp độ kết nối tắt được sử dụng.
        n_classes: Số lượng kênh đầu ra cho phân đoạn.
        resnet: Các cấu hình cho mạng ResNetV2 lai.
    """
...
def build_r50_vit_b16_config(
    *,
    ...
    """
    Xây dựng cấu hình tĩnh cho mô hình TransUNet phiên bản R50-ViT-B_16.

    Các tham số đầu vào:
        n_classes: Số lượng kênh đầu ra của vùng phân đoạn.
        decoder_channels: Bộ 4 số nguyên đại diện cho số kênh của nhánh giải mã.
        n_skip: Số lượng kết nối tắt được decoder sử dụng (từ 0 đến 3).
        hidden_size: Kích thước ẩn của Transformer.
        mlp_dim: Kích thước ẩn của khối MLP trong Transformer.
        num_heads: Số đầu tự chú ý (Attention heads).
        num_layers: Số lượng khối Transformer chồng lên nhau.
        dropout_rate: Tỷ lệ dropout sau embedding và MLP.
        attention_dropout_rate: Tỷ lệ dropout trong cơ chế chú ý.
        skip_channels: Bộ 4 số nguyên đại diện cho số kênh của các kết nối tắt.

    Kết quả trả về:
        Đối tượng cấu hình TransUNetConfig hoàn chỉnh.
    """
...
def _np_to_torch(weights: np.ndarray, conv: bool = False) -> torch.Tensor:
    """Chuyển đổi trọng số numpy sang Tensor PyTorch (hoán vị chiều HWIO->OIHW nếu là lớp Conv)."""
    if conv:
...
def _copy_param(param: nn.Parameter, value: torch.Tensor) -> None:
    """Sao chép giá trị tensor vào tham số mô hình sau khi đồng bộ hóa kiểu dữ liệu và thiết bị phần cứng."""
    param.copy_(value.to(device=param.device, dtype=param.dtype))


class StdConv2d(nn.Conv2d):
    """Lớp tích chập 2 chiều chuẩn hóa trọng số (Weight-standardized Conv2d) sử dụng trong ResNet lai."""
...
def _conv3x3(cin: int, cout: int, stride: int = 1, bias: bool = False) -> StdConv2d:
    """Hàm tạo nhanh lớp tích chập chuẩn hóa kích thước kernel 3x3."""
...
def _conv1x1(cin: int, cout: int, stride: int = 1, bias: bool = False) -> StdConv2d:
    """Hàm tạo nhanh lớp tích chập chuẩn hóa kích thước kernel 1x1."""
    return StdConv2d(cin, cout, kernel_size=1, stride=stride, padding=0, bias=bias)


class PreActBottleneck(nn.Module):
    """Khối Bottleneck tiền kích hoạt (Pre-activation ResNetV2 bottleneck block)."""
    ...
    def load_from(self, weights: Any, n_block: str, n_unit: str) -> None:
        """Tải trọng số của khối từ checkpoint file npz của TransUNet gốc."""
        with torch.no_grad():
            _copy_param(
                self.conv1.weight,
...
class ResNetV2(nn.Module):
    """Mạng ResNetV2 dùng để trích xuất đặc trưng lai (hybrid embeddings) cho TransUNet."""
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        for i in range(len(self.body) - 1):
            ...
            if x.size(2) != right_size:
                ...
                if pad >= 3 or pad <= 0:
                    raise ValueError(
                        f"Kích thước đặc trưng bất thường {x.shape}; mong đợi kích thước không gian {right_size}"
                    )
                feat = torch.zeros(
...
class Attention(nn.Module):
    """Cơ chế tự chú ý đa đầu (Multi-head self-attention) được dùng trong các khối mã hóa ViT."""
...
class Mlp(nn.Module):
    """Khối mạng MLP đa tầng trong kiến trúc Transformer."""
...
class Embeddings(nn.Module):
    """Xây dựng patch và vị trí nhúng (position embeddings) cho khối Transformer của TransUNet."""
    ...
    def __init__(
        ...
        if config.patches_grid is not None:
            ...
            if patch_size[0] <= 0 or patch_size[1] <= 0:
                raise ValueError(
                    "Kích thước patch của TransUNet không hợp lệ dựa trên lưới phân chia patch và ảnh gốc. "
                    f"Ảnh: {img_size}, Lưới: {grid_size}."
                )
            patch_size_real = (patch_size[0] * 16, patch_size[1] * 16)
            n_patches = (img_size[0] // patch_size_real[0]) * (img_size[1] // patch_size_real[1])
            self.hybrid = True
        else:
            raise ValueError("TransUNet yêu cầu cấu hình tham số patches_grid lai.")
...
class Block(nn.Module):
    """Khối Transformer đơn lẻ gồm các bước chuẩn hóa trước (pre-norm), tự chú ý và mạng MLP."""
    ...
    def load_from(self, weights: Any, n_block: int) -> None:
        """Tải trọng số cho khối encoder Transformer từ tệp numpy npz."""
...
class Encoder(nn.Module):
    """Tập hợp nhiều khối mã hóa Transformer (Stacked transformer blocks)."""
...
class Transformer(nn.Module):
    """Khối kết hợp trích xuất đặc trưng lai (hybrid embeddings) và mã hóa Transformer."""
...
class Conv2dReLU(nn.Sequential):
    """Khối tích chập 2 chiều kết hợp chuẩn hóa Batch Normalization và kích hoạt ReLU."""
...
class DecoderBlock(nn.Module):
    """Khối giải mã tăng kích thước ảnh kèm kết nối tắt tùy chọn (Upsampling decoder block)."""
...
class SegmentationHead(nn.Sequential):
    """Lớp phân loại tích chập cuối cùng để ánh xạ các đặc trưng ra ảnh phân đoạn."""
...
class DecoderCup(nn.Module):
    """Nhánh giải mã kiểu mạng U-Net cho TransUNet."""
    ...
    def forward(
        ...
        if h * w != n_patch:
            raise ValueError(
                "Mạng TransUNet yêu cầu lưới token phải là hình vuông. "
                f"Giá trị nhận được n_patch={n_patch}, không thể biến đổi thành hình vuông."
            )
...
class TransUNet(nn.Module):
    """
    Mạng phân đoạn hình ảnh TransUNet phục vụ phân đoạn nhị phân các vùng tổn thương da.

    Các tham số đầu vào:
        config: Cấu hình kiến trúc của mô hình.
        img_size: Kích thước của ảnh đầu vào (H, W) hoặc một số nguyên đại diện cho ảnh vuông.
        in_channels: Số lượng kênh dữ liệu đầu vào.
        vis: Cho phép lưu trữ bản đồ tự chú ý (attention maps) để trực quan hóa.
    """
    ...
    def _resize_positional_embeddings(
        ...
        """Thay đổi kích thước nhúng vị trí (position embeddings) bằng thuật toán nội suy song tuyến tính."""
        if posemb.ndim != 3:
            raise ValueError(
                f"Kỳ vọng posemb có kích thước (1, N, C), giá trị nhận được {tuple(posemb.shape)}"
            )
        ...
        if gs_old * gs_old != posemb.size(1) or gs_new * gs_new != ntok_new:
            raise ValueError(
                "Không thể thay đổi kích thước nhúng vị trí do lưới token không phải là hình vuông: "
                f"tokens_cũ={posemb.size(1)}, tokens_mới={ntok_new}."
            )
    ...
    def load_pretrained_from_npz(self, checkpoint_path: str | Path) -> None:
        """
        Tải trọng số mô hình ViT định dạng npz chính thức cho R50-ViT-B_16.

        Tham số đầu vào:
            checkpoint_path: Đường dẫn tới file chứa trọng số huấn luyện sẵn ``R50+ViT-B_16.npz``.

        Ngoại lệ:
            ValueError: Nếu thiếu các trường thông tin cần thiết hoặc cấu trúc hình học không khớp.
        """
        ckpt_path = Path(checkpoint_path)
        if not ckpt_path.exists():
            raise ValueError(f"Không tìm thấy file checkpoint tiền huấn luyện của TransUNet: {ckpt_path}")

        try:
            weights = np.load(str(ckpt_path), allow_pickle=False)
        except Exception as exc:
            raise ValueError(f"Không thể đọc file checkpoint tiền huấn luyện của TransUNet: {ckpt_path}") from exc
        ...
        try:
            ...
            raise ValueError(
                "Checkpoint của TransUNet bị thiếu khóa bắt buộc. "
                f"Yêu cầu checkpoint chuẩn định dạng R50+ViT-B_16.npz, thiếu khóa: {exc}"
            ) from exc
